...
$category_name = '';
$sql_cat_name = "SELECT category_name FROM category WHERE category_id = ?";
if ($stmt_cat_name = mysqli_prepare($conn, $sql_cat_name)) {
    mysqli_stmt_bind_param($stmt_cat_name, "i", $category_id); mysqli_stmt_execute($stmt_cat_name);
    $result_cat_name = mysqli_stmt_get_result($stmt_cat_name);
    if ($row = mysqli_fetch_assoc($result_cat_name)) {
        $category_name = $row['category_name'];
    } else { echo "Category not found."; exit(); }
    mysqli_stmt_close($stmt_cat_name);
}

// 2. Fetch Subcategories for this Category
$subcategories = [];
$sql_subcategories = "SELECT subcategory_id, subcategory_name FROM subcategory WHERE category_id = ?";
if ($stmt_sub = mysqli_prepare($conn, $sql_subcategories)) {
    mysqli_stmt_bind_param($stmt_sub, "i", $category_id); mysqli_stmt_execute($stmt_sub);
    $result_sub = mysqli_stmt_get_result($stmt_sub);
    while ($row = mysqli_fetch_assoc($result_sub)) { $subcategories[] = $row; }
...
// FETCH SHOPS AVAILABLE IN THIS CATEGORY FOR THE FILTER
$shops_for_filter = [];
$sql_shops = "SELECT DISTINCT s.shop_id, s.shop_name 
              FROM shops s 
              JOIN products p ON s.shop_id = p.shop_id 
              WHERE p.category_id = ? AND p.status = 'active'
              ORDER BY s.shop_name";
if ($stmt_shops = mysqli_prepare($conn, $sql_shops)) {
    mysqli_stmt_bind_param($stmt_shops, "i", $category_id);
    mysqli_stmt_execute($stmt_shops);
    $result_shops = mysqli_stmt_get_result($stmt_shops);
    while ($row = mysqli_fetch_assoc($result_shops)) {
        $shops_for_filter[] = $row;
    }
    mysqli_stmt_close($stmt_shops);
}

// DYNAMICALLY BUILD THE PRODUCTS QUERY
$sql_products = "SELECT p.product_id, p.product_name, p.price, p.product_image, p.created_at FROM products p WHERE p.category_id = ? AND p.status = 'active'";
$params = [$category_id];
$types = 'i';
...
<head>
    ...
    <title><?php echo htmlspecialchars($category_name); ?> | Vastra</title>
    ...
    <style>
        ...
        .page-header { text-align: center; padding: 2rem 1rem; background-color: #f9f9f9; border-bottom: 1px solid #eee; }
        .page-header h1 { margin: 0; font-size: 2.5rem; color: var(--dark-gray); }
        .subcategory-nav { display: flex; justify-content: center; gap: 1.5rem; padding: 1.5rem 0; margin-bottom: 2rem; border-bottom: 1px solid #ddd; flex-wrap: wrap; }
        .subcategory-nav a { text-decoration: none; color: var(--dark-gray); font-weight: 500; padding: 0.5rem 1rem; border-radius: 20px; transition: background-color 0.3s, color 0.3s; }
        .subcategory-nav a:hover, .subcategory-nav a.active { background-color: var(--primary); color: white; }
        .main-content-area { display: flex; gap: 2rem; align-items: flex-start; margin-top: 2rem; }
...
<body>
    ...
    <div class="page-header"><h1><?php echo htmlspecialchars($category_name); ?></h1></div>

    <div class="container">
        <?php if (!empty($subcategories)): ?>
        <nav class="subcategory-nav">
            <a href="category-products.php?id=<?php echo $category_id; ?>" class="active">All</a>
            <?php foreach ($subcategories as $sub): ?>
                <a href="subcategory-products.php?id=<?php echo $sub['subcategory_id']; ?>"><?php echo htmlspecialchars($sub['subcategory_name']); ?></a>
            <?php endforeach; ?>
        </nav>
        <?php endif; ?>
        ...
        <div class="main-content-area">
            <aside class="filter-sidebar">
                <form id="filter-form" method="GET" action="category-products.php">
                    ...
                    <?php endif; ?>
                    <a href="category-products.php?id=<?php echo $category_id; ?>" class="reset-filters">Reset All</a>
                </form>
            </aside>
    ...
    <div id="login-modal" class="modal-overlay">
        <div class="modal-content"><span class="close-button">&times;</span>
            ...
            <p>You need to be logged in to add items to your cart or wishlist.</p><a href="login.php"
                class="btn-modal-login">Login or Sign Up</a>
        </div>
    </div>

    <script>
        // Paste the full JavaScript block from index.php here. It's identical.
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function addToCart(productId, buttonElement) {
                ...
                if (result.status === 'success') {
                    ...
                    buttonElement.disabled = false; buttonElement.textContent = 'Add to Bag';
                }
            }
            ...
            document.addEventListener('click', (event) => {
                if (profileCard && profileCard.classList.contains('active') && !profileCardContainer.contains(event.target)) {
