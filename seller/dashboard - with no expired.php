...
$expiry_warning_threshold = 7; // Show warning if plan expires in 7 days or less

// Verify user is a seller
$stmt_role = $conn->prepare("SELECT role FROM login WHERE user_id = ?");
$stmt_role->bind_param("i", $user_id);
$stmt_role->execute();
$login_data = $stmt_role->get_result()->fetch_assoc();
$stmt_role->close();

if (!$login_data || $login_data['role'] !== 'seller') {
    session_destroy();
...
// --- 2. FETCH SELLER DATA ---
$stmt_user = $conn->prepare("SELECT CONCAT(fname, ' ', lname) full_name, email FROM users WHERE user_id = ?");
...
if ($subscription_result->num_rows > 0) {
    ...
    if ($end_date >= $today) {
        ...
        if ($days_remaining <= $expiry_warning_threshold) {
            $show_expiry_warning = true;
        }
    } else {
        $days_remaining = 0;
        $show_expiry_warning = true; // Also show warning if expired
    }
}
$stmt_sub->close();

// Check if the expiry alert has been dismissed in the current session
$show_expiry_alert_popup = $show_expiry_warning && !isset($_SESSION['expiryAlertDismissed']);
...
// --- 5. FETCH DATA FOR TABLES AND ALERTS ---
...
<head>
  ...
  <style>
    ...
    .alert-modal h2 { color: #e67e22; margin-top: 0; margin-bottom: 15px; font-size: 1.8rem; }
...
<body>
  ...
  <div class="main">
    ...
    <div class="card-container">
        <!-- Subscription Expiry Card (shows only when expiring) -->
        <?php if ($show_expiry_warning): ?>
            <a href="subscription.php" class="card-link">
                <div class="card <?php echo ($days_remaining > 0) ? 'warning' : 'danger'; ?>">
  ...
  <div class="modal-overlay" id="stock-alert-overlay">
      <div class="alert-modal">
          ...
          <h2><i class="fas fa-exclamation-triangle"></i> Inventory Alert</h2>
  ...
  <div class="modal-overlay" id="expiry-alert-overlay">
      <div class="alert-modal">
          ...
          <h2><i class="fas fa-clock"></i> Subscription Alert</h2>
          <?php if ($subscription_info): ?>
              <?php if ($days_remaining > 0): ?>
              ...
              <?php else: ?>
                <p>Your subscription has <strong>expired</strong>. Please renew now to restore your seller privileges.</p>
              <?php endif; ?>
          <?php endif; ?>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function closeStockModal() {
            ...
            fetch('dismiss_alert.php', { method: 'POST' });
        }
        ...
        document.querySelectorAll('.modal-overlay').forEach(overlay => {
            overlay.addEventListener('click', function(e) {
                if (e.target === this) {
                    if(this.id === 'stock-alert-overlay') closeStockModal();
                    if(this.id === 'expiry-alert-overlay') closeExpiryModal();
                }
            });
        });
    });
